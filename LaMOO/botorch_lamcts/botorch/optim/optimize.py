...
import json
import torch
from botorch_lamcts.botorch.acquisition.acquisition import (
    AcquisitionFunction,
    OneShotAcquisitionFunction,
)
from botorch_lamcts.botorch.acquisition.knowledge_gradient import qKnowledgeGradient
from botorch_lamcts.botorch.generation.gen import gen_candidates_scipy
from botorch_lamcts.botorch.logging import logger
from botorch_lamcts.botorch.optim.initializers import (
    gen_batch_initial_conditions,
    gen_one_shot_kg_initial_conditions,
)
from botorch_lamcts.botorch.optim.stopping import ExpMAStoppingCriterion
...
def optimize_acqf(
    ...
    **kwargs: Any,
    ...
    #     nasbench = json.load(f)
    #
    # with open('nasbench201_searched', 'r') as f:
    ...
    # full_archs = []
    # full_acqs = []
    # for arch in nasbench:
    #     if str([eval(arch)]) not in searched:
    #         full_archs.append([eval(arch)])
    # print('cur_length is', len(full_archs))
    #
    # for i in range(0, len(full_archs), 625):
    #     if torch.cuda.is_available():
    #         cur_arch = torch.tensor(full_archs[i: i+625], device='cuda')
    #     else:
    #         cur_arch = torch.tensor(full_archs[i: i + 625])
    #     # print('cur_arch is', cur_arch)
    #     cur_acq = acq_function(cur_arch)
    #     # print('cur_acq is', cur_acq)
    #     full_acqs.append(cur_acq)
    #
    # full_acqs = torch.cat(full_acqs)
    #
    # print('best full acqs is', full_acqs)
    # best = torch.argmax(full_acqs.view(-1), dim=0)
    #
    # searched[str(full_archs[best])] = True
    #
    # if torch.cuda.is_available():
    #     full_archs = torch.tensor(full_archs[best], device='cuda')
    # else:
    #     full_archs = torch.tensor(full_archs[best])
    #
    # full_acqs = full_acqs[best]
    #
    # with open('nasbench201_searched', 'w') as f:
    #     json.dump(searched, f)
    #
    # print('best full archs is', full_archs)
    # print('best full acqs is', full_acqs)
    #
    #
    # return full_archs, full_acqs
    ...
    # print('batch_candidates is', batch_candidates)
