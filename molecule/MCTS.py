...
class MCTS:
    ...
    def init_train(self):
        ...
        if not torch.cuda.is_available():
            ...
            # data = torch.load('molecule_obj3_data.pt', map_location=torch.device('cpu'))
        else:
            data = torch.load('molecule_data.pt')
            # data = torch.load('molecule_obj4_data.pt')
            # data = torch.load('molecule_obj3_data.pt')
...
#        print('init_obj is', init_obj)
    ...
    def search(self):
        ...
        for i in range(0, self.args.iter):
            ...
            for j in range(0, 1):
                # TODO: to replace with Bayesian Optimization, we use random for now
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cp', type=float, default=0.0, help='cp value in MCTS')
