...
def gen_batch_initial_conditions(
    ...
    lb = bounds[0].data.numpy()
    ub = bounds[1].data.numpy()

    while factor < max_factor:
        with warnings.catch_warnings(record=True) as ws:
            n = raw_samples * factor
            if lamcts_boundry is None or len(lamcts_boundry) == 0:
                ...
                while len(selected_cands) < n + 1:
                    ...
                    X_rnd = from_unit_cube(X_rnd , lb, ub)

                    # print('X_rnd.shape here is , q is', X_rnd.shape, q)
                    # X_rnd = X_rnd.reshape(X_rnd_shape[0], -1)
                    # X_rnd = X_rnd.cpu().detach().numpy()
                    # print('X_rnd here is', X_rnd)
                    for node in lamcts_boundry:
                        boundary = node[0].classifier.svm
                        if len(X_rnd) == 0:
                            break
                        try:
                            # print('good X_rnd is', X_rnd.shape)
                            X_rnd = X_rnd[boundary.predict(X_rnd) == node[1]]
                        except:
                            raise ValueError
                        # if len(selected_cands) == 1:
                        #     # print('cur_node is', node[0].get_name(), len(X_rnd))
                    selected_cands = np.append(selected_cands, X_rnd, axis=0)
                    # print('current len is', len(selected_cands))
                ...
                # final_cands_idx = np.random.choice(len(selected_cands), min(n, num_restarts + cur_iter * q) )
                # final_cands_idx = np.random.choice(len(selected_cands), n)
                final_cands_idx = np.random.choice(len(selected_cands), num_restarts)
                selected_cands = selected_cands[final_cands_idx]
                # print('current len is', len(selected_cands))
                X_rnd = torch.tensor(selected_cands, requires_grad=True)

                X_rnd = X_rnd.reshape(X_rnd.shape[0], -1, problem.dim)

            # print('X_rnd is', X_rnd)
            # return X_rnd
            ...
            # if check:
            #     print('check here is', batch_initial_conditions)
            #     print('X_rnd here is', X_rnd)
            if not any(issubclass(w.category, BadInitialCandidatesWarning) for w in ws):
                # print('bingo!!!!!!')
                # print('final output is:', batch_initial_conditions)
