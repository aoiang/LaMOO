...
class HIVProp:
    def __init__(self):
        ...
        # print('base_path is ===========================', os.path.join(base_path, 'hiv'))
...
class MOOMoleculeFunction:
    ...
    def __init__(self, props, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        if torch.cuda.is_available():
            self.ref_point = torch.tensor([0.0, 0.0, 0.0], device='cuda')
        else:
            self.ref_point = torch.tensor([0.0, 0.0, 0.0])
    ...
    def __call__(self, sample): 
        ...
        try:
            ...
            if torch.cuda.is_available():
                return torch.tensor([[0.0, 0.0, 0.0]], dtype=torch.float, device='cuda')
            else:
...
#
#     torch.save(data, 'molecule_obj3_data.pt')
