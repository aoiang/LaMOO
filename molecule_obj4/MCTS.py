...
from moo_molecule_funcs.properties_obj4 import MOOMoleculeFunction
from moo_molecule_funcs.properties_obj4 import SUPPORTED_PROPERTIES
...
class MCTS:
    ...
    def init_train(self):
        ...
        if not torch.cuda.is_available():
            # data = torch.load('molecule_data.pt', map_location=torch.device('cpu'))
            data = torch.load('molecule_obj4_data.pt', map_location=torch.device('cpu'))
        else:
            # data = torch.load('molecule_data.pt')
            data = torch.load('molecule_obj4_data.pt')
    ...
    def search(self):
        ...
        for i in range(0, self.args.iter):
            ...
            for j in range(0, 1):
                ...
                if not torch.cuda.is_available():
                    new_x_random = torch.tensor(selected_cands[final_cands_idx])
                else:
                    new_x_random = torch.tensor(selected_cands[final_cands_idx], device='cuda')
                new_obj_random = self.func(new_x_random)



                self.random_samples[0] = torch.cat([self.random_samples[0], new_x_random])
                self.random_samples[1] = torch.cat([self.random_samples[1], new_obj_random])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--problem', type=str, default='molecule', help='choose the problem')
    ...
    parser.add_argument('--iter', type=int, default=85, help='total iterations')
    ...
    parser.add_argument('--cp', type=float, default=0.1, help='cp value in MCTS')
    parser.add_argument('--sample_method', type=str, default='bayesian', help='bayesian, cmaes or random')
    parser.add_argument('--cmaes_method', type=str, default='vanilla', help='lamcts or vanilla')
    parser.add_argument('--split_method', type=str, default='regressor', help='dominance or regressor')
    ...
    if args.data_id < 0:
        for i in range(args.runs):
            if not torch.cuda.is_available():
                agent = MCTS(lb=f.bounds[0].data.numpy(), ub=f.bounds[1].data.numpy(), dims=f.dim, ninits=150, func=f,
                             args=args, run=i)
            else:
                agent = MCTS(lb=f.bounds[0].cpu().data.numpy(), ub=f.bounds[1].cpu().data.numpy(), dims=f.dim,
                             ninits=150, func=f, args=args,
                             run=i)
            agent.search()
    else:
        if not torch.cuda.is_available():
            ...
            agent = MCTS(lb=f.bounds[0].cpu().data.numpy(), ub=f.bounds[1].cpu().data.numpy(), dims=f.dim, ninits=150,
                         func=f, args=args,
                         run=args.data_id)
        agent.search()
