...
class MCTS:
    ...
    def init_train(self):
        ...
        if not torch.cuda.is_available():
            data = torch.load('VehicleSafety_data.pt', map_location=torch.device('cpu'))
        else:
            data = torch.load('VehicleSafety_data.pt')
    ...
    def search(self):
        ...
        for i in range(0, self.args.iter):
            ...
            for j in range(0, 1):
                ...
                # samples = leaf.propose_samples_Bayesian_Optimization(sample_num, path, self.lb, self.ub, self.surrogate_model, self.mll, train_obj, self.func, acq_func='qparego')
